...
if date_avl==True:
    col1,col2 = st.columns(2)
    with col1:
        normalspeed = [x for x in df_new_data['speed (kmph)'] if x>3 and x<=45 and math.isnan(x)==False]
    ...
    col1,col2 = st.columns(2)
    with col1:
        idletime = [x for x in df_new_data['speed (kmph)'] if x<2 and math.isnan(x)==False]
        runtim = [x for x in df_new_data['speed (kmph)'] if x>2 and math.isnan(x)==False]
    ...
    with col2:
        ...
        st.download_button(label='Download',data=df_new_data[df_new_data['speed (kmph)']>2].to_csv().encode('utf-8'),file_name='Bus_Activity.csv')
        st.dataframe(df_new_data[df_new_data['speed (kmph)']>2],use_container_width=True)
...
st.header('Service Uptime of Tracker')
st.subheader(str((df_new_data.shape[0]/(df_new_data.shape[0]+df_new_error.shape[0]))*100)+'%')
